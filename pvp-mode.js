...
window.startPvPBattle = function (matchId) {
  ...
  window._pvpCardsRevealed = false; // Reset flag para nueva batalla
  ...
  pvpRealtimeChannel = sb
    ...
    .on(
      ...
      (payload) => {
        ...
        // FASE 1: Si ambos jugadores jugaron sus cartas, REVELAR
        if (match.player1_card && match.player2_card && !window._pvpCardsRevealed) {
          console.log('✅ Ambos jugadores tienen cartas - REVELANDO');
          window._pvpCardsRevealed = true;
          if (typeof window.revealCardsFromSupabase === 'function') {
            window.revealCardsFromSupabase(match);
          }
        }

        // FASE 2: Si tenemos resultado, procesar victoria/derrota
        if (
          match.player1_card && 
          match.player2_card && 
...
/* ═══════════════════════════════════════════════════════════
   REVELAR CARTAS DESDE SUPABASE (NUEVO)
═══════════════════════════════════════════════════════════ */
...
window.revealCardsFromSupabase = function (match) {
  ...
  // Obtener todas las cartas disponibles desde index.html
  const allCards = typeof window.ALL_CARDS !== 'undefined' ? window.ALL_CARDS : [];
  
  if (!Array.isArray(allCards) || allCards.length === 0) {
    console.warn('⚠️ ALL_CARDS no disponible');
    return;
  }

  // Buscar la carta del jugador 1 por nombre
  const p1Card = allCards.find(c => c.name === match.player1_card);
  
  // Buscar la carta del jugador 2 por nombre
  const p2Card = allCards.find(c => c.name === match.player2_card);

  console.log('🃏 Cartas encontradas:', { 
    p1: p1Card?.name || 'NO ENCONTRADA', 
    p2: p2Card?.name || 'NO ENCONTRADA'
  });

  if (p1Card && p2Card) {
    // Actualizar variables globales para que doCombat() las encuentre
    if (pvpPlayerRole === 'player1') {
      window.selectedCard = p1Card;
      window.aiSelectedCard = p2Card;
    } else {
      window.selectedCard = p2Card;
      window.aiSelectedCard = p1Card;
    }

    // Llamar a la función de reveal
    if (typeof window.revealCardsAuto === 'function') {
      console.log('✅ Llamando revealCardsAuto()');
      window.revealCardsAuto();
    } else {
      console.warn('⚠️ revealCardsAuto no disponible');
    }
  } else {
    console.warn('⚠️ No se encontraron las cartas en ALL_CARDS', {
      buscó: match.player1_card,
      buscó2: match.player2_card
    });
  }
};
