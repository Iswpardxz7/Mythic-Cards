...
window.acceptPvPInvitationById = async function (invId, matchId, fromId, fromName) {
  ...
  console.log('✅ Invitación aceptada, yendo a combate inmediatamente');
  alert(`✅ ¡Aceptaste! Entrando al combate...`);
...
window.revealCardsFromSupabase = function (match) {
  ...
  // Obtener cartas desde ALL_CARDS
  ...
  // Asignar cartas a variables globales ANTES de mostrar
  ...
  console.log('✅ Cartas asignadas:', { selected: window.selectedCard?.name, ai: window.aiSelectedCard?.name });

  // Cambiar estado del juego a 'ready' (importante para revealCardsAuto)
  if (typeof window.gamePhase !== 'undefined') {
    window.gamePhase = 'ready';
  }

  // Mostrar ambas cartas con dorso (revealed=false)
  if (typeof window.renderSlots === 'function') {
    console.log('🖼️ Renderizando slots con dorso');
    window.renderSlots(p1Card, p2Card, false, false);
  }
  ...
  if (typeof window.startAutoRevealTimer === 'function') {
    console.log('⏱️ Iniciando auto-reveal timer');
    window.startAutoRevealTimer();
  } else {
    console.warn('⚠️ startAutoRevealTimer no disponible');
  }
};
